-- B1: fan-out in-app notifications on every order status change.
-- Customer follows their order, merchant learns of new orders, riders learn of open jobs.
...
BEGIN
  ...
  -- Open job broadcast to available riders (cap fan-out).
  IF NEW.status IN ('ร้านค้ารับออร์เดอร์', 'กำลังเตรียมสินค้า') AND NEW.rider_id IS NULL THEN
    INSERT INTO public.mobile_notifications(recipient_id, recipient_role, title, body, data, status, created_at, sent_at)
...
DROP TRIGGER IF EXISTS notify_order_status_after_change ON public.delivery_orders;
CREATE TRIGGER notify_order_status_after_change
